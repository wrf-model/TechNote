...
the ARW solver as $\Phi = (U,V,W,\Theta,\phi', \mu', Q_m)$ and the 
...
terms) in equations \eqref{u-mom-pert} -- \eqref{scalar-pert}, and
$R(\Phi)$ are the remaining terms in \eqref{u-mom-pert} --
\eqref{scalar-pert}.
...
\Omega'' = \Omega - \Omega^{t^*}, ~~~~~~ \Theta'' = \Theta -
\Theta^{t^*}, \notag \\ \phi'' &= \phi' - {\phi'}^{t^*}, 
...
%
\begin{equation} p'' = {c_s^2\over\alpha_d^{t^*}} \biggl({\Theta''\over\Theta^{t^*}}
...
are substituted into the prognostic equations
\eqref{u-mom-pert} -- \eqref{scalar-pert} and lead to the 
...
and an acoustic time-step averaging operator 
%
\begin{equation}
...
\eqref{mu-small-step} 
to integrate the
$\partial_\eta \Omega''$ term vertically using the lower boundary 
condition $\Omega''=0$ at the
surface. Equation \eqref{theta-small-step} is then stepped forward to calculate
${\Theta''}^{\tau+\Delta\tau}$. Equations \eqref{w-small-step} and 
...
%
\hphantom{BeginBegin} 
(7) Scalar transport: Advance scalars \eqref{scalar-pert} 
...
\eqref{moist-state-equation} and \eqref{hydro-pert}
\medskip \hfill \break
%
...
\ref{time_integration_figure}, the RHS term $R_\Theta^{t^*}$ in the
...
$R_\Theta^{t^*}$ from \eqref{theta-small-step} in the acoustic
...
estimate of the diabatic forcing of $\Theta$ arising from the non-RK3
physics in step (9) is included in the diabatic forcing term
$R_\Theta^{t^*}$ in \eqref{theta-small-step} (which is advanced in step
4).  This estimated diabatic forcing is then removed from the updated
$\Theta$ after the RK3 integration is complete and before the evaluation
...
\ref{figure:2} are the column mass $\mu$, defined at the $(i,j)$ points
(mass points) on the discrete grid, the geopotential $\phi$ that is
defined at the $w$ points, and the moisture variables $q_m$ are defined
at the mass points.  The diagnostic variables used in the model, the
