...
WRF Preprocessng System, referred to as WPS) that converts
the large-scale GRIB data into a format suitable for ingest by ARW's
real-data processor.
...
\item input data that are on the correct horizontal and vertical staggering;
...
that utilizes a binary file providing a 2D profile specified in $[y,z]$, the
...
The output data from WPS supply a complete 3-dimensional snapshot of the atmosphere
...
the data flow and program components in WPS, and how WPS feeds initial data to ARW.
...
\noindent The isobaric temperature and the stratospheric correction supply a reasonable 
reference temperature up to approximately 100 Pa. From the reference pressure and the final reference temperature, 
...
The input data from WPS contain both a total pressure and a moisture field (typically
relative humidity).  Starting at the top of each column of input pressure data, the integrated moisture
...
\noindent which would take a grid point from the initial value to the value at the next boundary update time 
