...
from the coarse grid forecast.  In a 1-way nest, the only
...
the coarse grid is integrated first and once the coarse-grid forecast is completed, 
...
tracks a center of lowest pressure), 
...
imbedded with a nest domain using a 3:1 grid size ratio.  Solid lines
...
In order to instantiate a grid and to provide 
time-dependent lateral boundaries, the horizontal interpolation does not conserve mass.
The feedback mechanism uses cell averages for most of the unmasked fields 
(for mass/thermodynamic/scalar/chemistry quantities), and uses cell-face 
