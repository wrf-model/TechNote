...
(Lambert conformal, polar stereographic, and Mercator), the map-scale factors are the same in both horizontal directions, such that $m_x=m_y=m$. For the anisotropic latitude-longitude grid, $m_x=\sec\psi$ and $m_y=1$, so that $\partial m_x/\partial y  = (1/r_e) \sec \psi \tan \psi$ and $\partial m_y/\partial x  = 0$, 
